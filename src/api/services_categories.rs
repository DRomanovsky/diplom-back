...
pub async fn update_services_categories_by_id(db: web::Data<Database>, id: web::Path<String>, mut payload: Multipart) -> HttpResponse {
    ...
    let updated_services_categories: ServiceCategory = serde_json::from_value(json_data.get("updated_services_categories").cloned().ok_or_else(|| MyError::CustomError("Missing 'updated_services_categories' field".to_string())).unwrap()).map_err(|err| MyError::CustomError(format!("Invalid 'updated_services_categories': {}", err))).unwrap();
    let image_path = dir.to_owned() + &filename.to_string().to_owned() + ".png";
